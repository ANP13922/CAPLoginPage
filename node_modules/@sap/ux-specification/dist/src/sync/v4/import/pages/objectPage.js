...
function getManifestId(sectionId, splitOnLt = false) {
    ...
    return `${pathParts[0]?.replace(/::/g, '/')}@${"com.sap.vocabularies.UI.v1.LineItem" /* LineItem */}${pathParts[1]?.replace(/::/g, '#')}`;
}
...
function fillSectionsInConfig(importParameters, sectionProperties, manifestSettings, configPart, factory) {
    for (const sectionId in sectionProperties) {
        ...
        if (sectionId.includes('com.sap.vocabularies.UI.v1.PresentationVariant') &&
            schemaSection.$ref?.startsWith(common_1.DEFINITION_LINK_PREFIX + common_1.DefinitionName.ObjectPageSectionTableV4)) {
        ...
        else if (schemaSection.$ref?.startsWith(common_1.DEFINITION_LINK_PREFIX + common_1.DefinitionName.ObjectPageSectionTableV4)) {
    ...
    if (headerSections && headerSections.properties) {
        ...
        for (const sectionId in headerSections.properties) {
            ...
            objectPageConfig.header.sections[sectionId] = factoryKey
                ...
                : {};
            utils_1.transferSettingsOfObject(objectPageConfig.header.sections[sectionId], manifest, sectionDefinition, routingId, [], sectionDefinition?.title?.split(common_1.FacetTitlePrefix)[1]);
        }
    }
}
/**
